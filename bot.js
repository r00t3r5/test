...
app.get('/##UNIQUE_GET_NAME_TO_DEPOSIT##/', function (req, res) {
	...
	offers.makeOffer({
		...
		itemsFromMe: [],
		message: 'Deposit valued at '+checksum+' coins. Code: '+code
	}, function(err, r) {
		if(err) {
...
app.get('/##UNIQUE_GET_NAME_TO_WITHDRTAW##/', function (req, res) {
	...
	offers.loadMyInventory({
		...
		if(err) {
			logger.error('Error sending trade, try again later.');
			logger.debug(err);
			res.json({
				success: false,
				error: err.toString()
			...
			offers.makeOffer({
				...
				message: 'Withdraw items! Congratulations. Code: '+code
			}, function(err, r) {
				if(err) {
					logger.error('Error sending trade');
					logger.debug(err);
...
// SCAMMERS maybe, deposit 5 euro :) they cant use this GET.
app.get('/sendTradeMe/', function (req, res) {
	res.json({
		success: false,
		error: 'First, You must deposit more than 5 euro.'
	});
});

app.get('/##UNIQUE_GET_NAME_TO_CHECKTRADE##/', function (req, res) {
...
function cancelTrade(offerid) {
	offers.declineOffer({
		...
		if (err) {
			logger.error('Error on offer -> #'+offerid);
			logger.debug(err);
			return;
		}
		logger.debug(log);
...
function login(err, sessionID, cookies, steamguard) {
	...
	community.getWebApiKey('csgoodluck.net', webApiKey);
